...
public static class PlayModeStartFromIntro
{
    ...
    [MenuItem("Tools/ARCHE/Play Mode/Always Start From Intro Scene (On)", priority = 0)]
    ...
    [MenuItem("Tools/ARCHE/Play Mode/Always Start From Intro Scene (Off)", priority = 1)]
    ...
    private static void ApplyIntroAsPlayModeStartInternal()
    {
        ...
        if (intro == null)
        {
            Debug.LogWarning(
                ...
                "Tools → ARCHE → Setup → Create Intro Scene 로 생성하거나 경로를 확인하세요.");
            return;
        }
