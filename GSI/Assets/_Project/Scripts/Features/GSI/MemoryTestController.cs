...
/// 연습은 1회 시도, 공식 시험은 1회 최대 길이로 점수화합니다.
/// </summary>
public sealed class MemoryTestController : MonoBehaviour, IMiniGameController
{
    public string GameId => "MemorySequence";
    ...
    private const int ExamTrialCount = 1;
    ...
    private void OnEnable()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            {
                _assignedGrade = GameManager.Instance.GetPracticeGrade(TestMode.MemorySequence);
            }
    ...
    {
        if (_recallHintText == null || !_recallHintText.gameObject.activeSelf)
        {
            return;
        }

        _recallHintText.text = GameLocalization.GetUiString(UiStringKeys.MemoryRecallHint,
            "Sequence shown, repeat in order");
    }
    ...
    private void HandleGameStateChanged(GameState newState)
    {
        ...
        if (newState == GameState.TestStandby || newState == GameState.TestBriefing || newState == GameState.MainMenu)
        {
            StopSession();
            SetGridVisible(false);
            SetOtherMinigameVisuals(true);
    ...
    private IEnumerator RunSessionRoutine()
    {
        ...
        if (GameManager.Instance != null && GameManager.Instance.CurrentTestType == TestType.UnifiedOfficialExam)
        {
            ...
            OnGameFinished?.Invoke(finalScore, true);
            _routine = null;
            yield break;
        }
    ...
    private IEnumerator PlayShowPhaseCoroutine()
    {
        ...
        SetRecallHintVisible(true,
            GameLocalization.GetUiString(UiStringKeys.MemoryRecallHint, "Sequence shown, repeat in order"));
    }
