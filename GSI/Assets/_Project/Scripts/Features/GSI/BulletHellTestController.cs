...
/// </summary>
public sealed class BulletHellTestController : MonoBehaviour, IMiniGameController
{
    public string GameId => "BulletHell";
    ...
    private void OnEnable()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            {
                _assignedGrade = GameManager.Instance.GetPracticeGrade(TestMode.BulletHell);
            }
    ...
    private IEnumerator RunSessionRoutine()
    {
        ...
        if (isUnified && GameManager.Instance != null)
        {
            ...
            OnGameFinished?.Invoke(sumSeconds, !failed);
            _sessionRoutine = null;
            yield break;
        }
    ...
    private void UpdateHud()
    {
        ...
        string inv = _invulnTimer > 0f
            ? GameLocalization.FormatUiString(UiStringKeys.BhHudInvincibilityFmt, "Invinc. {0}s",
                _invulnTimer.ToString("0.0", CultureInfo.InvariantCulture))
            : " ";

        string elapsedStr = _elapsedRound.ToString("0.#", CultureInfo.InvariantCulture);
        string targetStr = _targetSeconds.ToString("0.#", CultureInfo.InvariantCulture);
        if (_grade1SurvivalMode)
        {
            _hudText.text = GameLocalization.FormatUiString(UiStringKeys.BhHudLiveSurvivalFmt,
                "Time {0}s · lives {1} · {2}",
                elapsedStr, _lives.ToString(CultureInfo.InvariantCulture), inv);
        }
        else
        {
            _hudText.text = GameLocalization.FormatUiString(UiStringKeys.BhHudLiveTargetFmt,
                "Time {0}s / target {1}s · lives {2} · {3}",
                elapsedStr, targetStr, _lives.ToString(CultureInfo.InvariantCulture), inv);
        }
    }

    private void ApplyBulletHellRoundIntroHud()
    {
        if (_hudText == null)
        {
            return;
        ...
        string invS = BulletHellDifficulty.InvincibilitySeconds.ToString("0.#", CultureInfo.InvariantCulture);
        if (_grade1SurvivalMode)
        {
            _hudText.text = GameLocalization.FormatUiString(UiStringKeys.BhHudRoundIntroSurvivalFmt,
                "Tier 1 — max survival · lives {0} · invinc. {1}s",
                _lives, invS);
        }
        else
        {
            string targetS = _targetSeconds.ToString("0.#", CultureInfo.InvariantCulture);
            _hudText.text = GameLocalization.FormatUiString(UiStringKeys.BhHudRoundIntroTargetFmt,
                "Survive {0}s · lives {1} · invinc. {2}s",
                targetS, _lives.ToString(CultureInfo.InvariantCulture), invS);
        }
    }
