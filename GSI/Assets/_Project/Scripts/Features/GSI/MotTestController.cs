...
/// </summary>
public sealed class MotTestController : MonoBehaviour, IMiniGameController
{
    public string GameId => "MultipleObjectTracking";
    ...
    private enum MotHudLocalePhase
    {
        None,
        Memorize,
        Tracking,
    ...
    private MotHudLocalePhase _hudLocalePhase;
    ...
    private TextMeshProUGUI _submitLabelText;
    ...
    private void OnEnable()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            {
                _assignedGrade = GameManager.Instance.GetPracticeGrade(TestMode.MultipleObjectTracking);
            }
    ...
    {
        if (_submitLabelText != null)
        {
            _submitLabelText.text = GameLocalization.GetUiString(UiStringKeys.MotSubmit, "Submit");
        }

        switch (_hudLocalePhase)
        {
            case MotHudLocalePhase.Memorize:
                if (_hudText != null)
                {
                    _hudText.text = GameLocalization.GetUiString(UiStringKeys.MotHudMemorize,
                        "Memorize the highlighted targets.");
                }

                break;
            case MotHudLocalePhase.Tracking:
                if (_hudText != null)
                {
                    _hudText.text = GameLocalization.GetUiString(UiStringKeys.MotHudTracking,
                        "Tracking... (all shapes look the same)");
                }

                break;
            case MotHudLocalePhase.Select:
    ...
    private IEnumerator RunSessionRoutine()
    {
        ...
        if (GameManager.Instance != null && GameManager.Instance.CurrentTestType == TestType.UnifiedOfficialExam)
        {
            ...
            OnGameFinished?.Invoke(avgAcc, !failed);
            _sessionRoutine = null;
            yield break;
        }
    ...
    private IEnumerator RunSingleTrial(int seed)
    {
        ...
        if (_hudText != null)
        {
            _hudLocalePhase = MotHudLocalePhase.Memorize;
            _hudText.text = GameLocalization.GetUiString(UiStringKeys.MotHudMemorize, "Memorize the highlighted targets.");
        }
        ...
        if (_hudText != null)
        {
            _hudLocalePhase = MotHudLocalePhase.Tracking;
            _hudText.text = GameLocalization.GetUiString(UiStringKeys.MotHudTracking,
                "Tracking... (all shapes look the same)");
        }
        ...
        if (_hudText != null)
        {
            _hudLocalePhase = MotHudLocalePhase.Select;
            _hudText.text = GameLocalization.FormatUiString(UiStringKeys.MotHudSelectFmt,
                "Tap the targets you tracked (up to {0}). Press [Submit] anytime.", targets);
        }
    ...
    private void UpdateSelectionHudText()
    {
        ...
        _hudText.text = GameLocalization.FormatUiString(UiStringKeys.MotHudStatusFmt,
            "Selected {0} / {1}, [Submit] to confirm (partial OK)", _selected.Count, _targetCount);
    }
    ...
    private void BuildPlayArea(int totalObjects, int targets, int seed, float speed)
    {
        ...
        _submitLabelText = subTmp;
        subTmp.text = GameLocalization.GetUiString(UiStringKeys.MotSubmit, "Submit");
        subTmp.fontSize = 20;
    ...
    private void DestroyPlayRoot()
    {
        _motionActive = false;
        _trialSubmitPending = false;
        _hudLocalePhase = MotHudLocalePhase.None;
        _disks.Clear();
        _selected.Clear();
        _fieldRect = null;
        _submitButton = null;
        _hudText = null;
        _submitLabelText = null;
        if (_playRoot != null)
        {
