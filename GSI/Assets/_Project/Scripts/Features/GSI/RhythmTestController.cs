...
/// </summary>
public sealed class RhythmTestController : MonoBehaviour, IMiniGameController
{
    public string GameId => "RhythmTiming";
    ...
    private enum RhythmHudLocalePhase
    {
        None,
        Ready,
        Go,
    ...
    private RhythmHudLocalePhase _rhythmHudLocalePhase;
    ...
    private TextMeshProUGUI _keyHintText;
    ...
    private void OnEnable()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            {
                _assignedGrade = GameManager.Instance.GetPracticeGrade(TestMode.RhythmTiming);
            }
    ...
    private void SubscribeLocale()
    {
        ...
        GameLocalization.UiLocaleChanged += OnUiLocaleChanged;
    ...
    private void OnUiLocaleChanged()
    {
        if (_keyHintText != null)
        {
            _keyHintText.text = GetKeyHintText(_laneCount);
        }
        ...
        switch (_rhythmHudLocalePhase)
        {
            case RhythmHudLocalePhase.Ready:
                _hudText.text = GameLocalization.GetUiString(UiStringKeys.RhythmHudReady, "Ready...");
                break;
            case RhythmHudLocalePhase.Go:
                _hudText.text = GameLocalization.GetUiString(UiStringKeys.RhythmHudGo, "Go!");
                break;
            case RhythmHudLocalePhase.Playing:
                RefreshRhythmHudPlayingText();
                break;
        }
    }

    private void RefreshRhythmHudPlayingText()
    {
        if (_hudText == null)
        {
            return;
        ...
        _hudText.text = GameLocalization.FormatUiString(UiStringKeys.RhythmHudRecordingFmt,
            "Recording error (ms)...  Hits {0}/{1}", hits, done);
    }
    ...
    private IEnumerator RunSessionRoutine()
    {
        ...
        if (GameManager.Instance != null && GameManager.Instance.CurrentTestType == TestType.UnifiedOfficialExam)
        {
            ...
            OnGameFinished?.Invoke(meanErr, !failed);
            _sessionRoutine = null;
            yield break;
        }
    ...
    private IEnumerator RunSingleRound(int seed, bool isExam)
    {
        ...
        if (_hudText != null)
        {
            _rhythmHudLocalePhase = RhythmHudLocalePhase.Ready;
            _hudText.text = GameLocalization.GetUiString(UiStringKeys.RhythmHudReady, "Ready...");
        }
        ...
        if (_hudText != null)
        {
            _rhythmHudLocalePhase = RhythmHudLocalePhase.Go;
            _hudText.text = GameLocalization.GetUiString(UiStringKeys.RhythmHudGo, "Go!");
        }
        ...
        _roundActive = true;
        _rhythmHudLocalePhase = RhythmHudLocalePhase.Playing;
    ...
    private static string GetKeyHintText(int lanes)
    {
        switch (lanes)
        {
            case 4:
                return GameLocalization.GetUiString(UiStringKeys.RhythmKeysFourLanes, "Keys: D, F, J, K");
            case 3:
                return GameLocalization.GetUiString(UiStringKeys.RhythmKeysThreeLanes, "Keys: F, G, H");
            case 2:
                return GameLocalization.GetUiString(UiStringKeys.RhythmKeysTwoLanes, "Keys: D, F");
            default:
                return string.Empty;
        }
    }
    ...
    private void TickRound()
    {
        ...
        for (int i = 0; i < _noteInstances.Count; i++)
        {
            ...
            if (songTime > r.Data.Time + windowSec + 0.008f)
            {
                ...
                SpawnJudgmentPopup(laneIdx, GameLocalization.GetUiString(UiStringKeys.RhythmJudgeMiss, "Miss!"),
                    new Color(1f, 0.38f, 0.38f, 1f), new Color(1f, 0.2f, 0.2f, 0.5f));
            }
        }
    ...
    private void GetHitJudgmentStyle(float absErrorMs, out string label, out Color color)
    {
        ...
        if (ratio <= 0.22f)
        {
            label = GameLocalization.GetUiString(UiStringKeys.RhythmJudgePerfect, "T0");
            color = GsiUiAppearance.TextPrimary;
        }
        else if (ratio <= 0.5f)
        {
            label = GameLocalization.GetUiString(UiStringKeys.RhythmJudgeGreat, "T1");
            color = GsiUiAppearance.TextPrimary;
        }
        else
        {
            label = GameLocalization.GetUiString(UiStringKeys.RhythmJudgeGood, "T2");
            color = GsiUiAppearance.TextSecondary;
        }
    }
    ...
    {
        _rhythmHudLocalePhase = RhythmHudLocalePhase.None;
        _noteInstances.Clear();
        _laneRects = null;
        if (_playRoot != null)
        {
