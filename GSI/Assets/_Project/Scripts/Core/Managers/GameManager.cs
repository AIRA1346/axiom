...
public sealed class GameManager : MonoBehaviour
{
    ...
    public int UnifiedExamSegmentOrdinalDisplay
    {
        get
        {
            ...
            return Mathf.Clamp(_unifiedNextIndex + 1, 1, _unifiedOrder.Length);
        }
    }
    ...
    private bool _releasePersistenceSubscribed;
    private EconomyManager _subscribedEconomyManager;
    private PlayerDataManager _subscribedPlayerDataManager;
    private Coroutine _steamCloudPushCoroutine;
    private float _lastSteamCloudPushRealtime = -999f;

    private const float SteamCloudPushCooldownSeconds = 30f;
    ...
    private void Awake()
    {
        ...
        InitializeFirstLaunchResolution();
    ...
    private void InitializeFirstLaunchResolution()
    {
#if !UNITY_EDITOR && (UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX || UNITY_STANDALONE)
        const string ResInitKey = "GSI_HasInitializedResolution";
        if (GsiSaveSystem.GetInt(ResInitKey, 0) == 0)
        {
            Resolution current = Screen.currentResolution;
            if (current.width > 0 && current.height > 0)
            {
                Screen.SetResolution(current.width, current.height, Screen.fullScreenMode);
                Debug.Log($"[ResolutionInit] Initialized first launch resolution to native: {current.width}x{current.height} ({Screen.fullScreenMode})");
            }
            GsiSaveSystem.SetInt(ResInitKey, 1);
            GsiSaveSystem.Save();
        }
#endif
    }
    ...
    {
        int legacy = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradePrefsKey, 9), 1, 9);

        if (!GsiSaveSystem.HasKey(PracticeGradeReactionKey))
        {
        ...
        _practiceGradeReaction = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeReactionKey, 9), 1, 9);
        _practiceGradeAim = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeAimKey, 9), 1, 9);
        _practiceGradeMemory = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeMemoryKey, 9), 1, 9);
        if (!GsiSaveSystem.HasKey(PracticeGradeRhythmKey))
        {
            _practiceGradeRhythm = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradePrefsKey, 9), 1, 9);
        }
        else
        {
            _practiceGradeRhythm = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeRhythmKey, 9), 1, 9);
        }

        if (!GsiSaveSystem.HasKey(PracticeGradeMotKey))
        {
            _practiceGradeMot = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradePrefsKey, 9), 1, 9);
            SavePracticeGrades();
        }
        else
        {
            _practiceGradeMot = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeMotKey, 9), 1, 9);
        }

        if (!GsiSaveSystem.HasKey(PracticeGradeBulletHellKey))
        {
            _practiceGradeBulletHell = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradePrefsKey, 9), 1, 9);
            SavePracticeGrades();
        }
        else
        {
            _practiceGradeBulletHell = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeBulletHellKey, 9), 1, 9);
        }

        if (!GsiSaveSystem.HasKey(PracticeGradeCpsKey))
        {
            _practiceGradeCps = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradePrefsKey, 9), 1, 9);
            SavePracticeGrades();
        }
        else
        {
            _practiceGradeCps = Mathf.Clamp(GsiSaveSystem.GetInt(PracticeGradeCpsKey, 9), 1, 9);
        }
    }
    ...
    {
        GsiSaveSystem.SetInt(PracticeGradeReactionKey, _practiceGradeReaction);
        GsiSaveSystem.SetInt(PracticeGradeAimKey, _practiceGradeAim);
        GsiSaveSystem.SetInt(PracticeGradeMemoryKey, _practiceGradeMemory);
        GsiSaveSystem.SetInt(PracticeGradeRhythmKey, _practiceGradeRhythm);
        GsiSaveSystem.SetInt(PracticeGradeMotKey, _practiceGradeMot);
        GsiSaveSystem.SetInt(PracticeGradeBulletHellKey, _practiceGradeBulletHell);
        GsiSaveSystem.SetInt(PracticeGradeCpsKey, _practiceGradeCps);
        GsiSaveSystem.SetInt(PracticeGradePrefsKey, _practiceGradeReaction);
        GsiSaveSystem.Save();
    }
    ...
    private void Start()
    {
        ...
        SubscribeToReleasePersistenceSignals();
    ...
    {
        UnsubscribeFromInputManager();
        UnsubscribeFromReleasePersistenceSignals();
    ...
    private void SubscribeToReleasePersistenceSignals()
    {
        if (_releasePersistenceSubscribed)
        {
            return;
        }

        GsiUserSettings.SettingsChanged += HandleReleasePersistenceChanged;
        GsiUiAppearance.Changed += HandleReleasePersistenceChanged;
        GameLocalization.UiLocaleChanged += HandleReleasePersistenceChanged;
        PlayerCosmetics.CosmeticsChanged += HandleReleasePersistenceChanged;
        if (EconomyManager.Instance != null)
        {
            _subscribedEconomyManager = EconomyManager.Instance;
            _subscribedEconomyManager.OnEconomyChanged += HandleReleasePersistenceChanged;
        }

        if (PlayerDataManager.Instance != null)
        {
            _subscribedPlayerDataManager = PlayerDataManager.Instance;
            _subscribedPlayerDataManager.OnPlayerDataChanged += HandleReleasePersistenceChanged;
        }

        _releasePersistenceSubscribed = true;
    }

    private void UnsubscribeFromReleasePersistenceSignals()
    {
        if (!_releasePersistenceSubscribed)
        {
            return;
        ...
        GameLocalization.UiLocaleChanged -= HandleReleasePersistenceChanged;
        PlayerCosmetics.CosmeticsChanged -= HandleReleasePersistenceChanged;
        if (_subscribedEconomyManager != null)
        {
            _subscribedEconomyManager.OnEconomyChanged -= HandleReleasePersistenceChanged;
            _subscribedEconomyManager = null;
        }
        ...
        _releasePersistenceSubscribed = false;
    }

    private void HandleReleasePersistenceChanged()
    {
        QueueSteamCloudPush();
    }

    private void QueueSteamCloudPush()
    {
        ...
        float elapsed = Time.realtimeSinceStartup - _lastSteamCloudPushRealtime;
        if (elapsed >= SteamCloudPushCooldownSeconds)
        {
            FlushSteamCloudNow();
            return;
        }

        if (_steamCloudPushCoroutine == null)
        {
            _steamCloudPushCoroutine = StartCoroutine(FlushSteamCloudWhenCooldownAllows());
        }
    }

    private IEnumerator FlushSteamCloudWhenCooldownAllows()
    {
        float wait = Mathf.Max(0.5f, SteamCloudPushCooldownSeconds - (Time.realtimeSinceStartup - _lastSteamCloudPushRealtime));
        yield return new WaitForSecondsRealtime(wait);
        _steamCloudPushCoroutine = null;
        FlushSteamCloudNow();
    }

    private void FlushSteamCloudNow()
    {
        GsiSteamCloudSync.TryPushLocalToCloud();
        _lastSteamCloudPushRealtime = Time.realtimeSinceStartup;
    }
    ...
    /// <summary>통합 공식 시험을 시작합니다.</summary>
    public bool TryStartUnifiedExam(int examGrade1to9)
    {
        ...
        var activeGames = ArchE.Game.MiniGameRegistry.Instance != null 
            ? ArchE.Game.MiniGameRegistry.Instance.ActiveMiniGames 
            : null;

        if (activeGames != null && activeGames.Count > 0)
        {
            _unifiedOrder = new TestMode[activeGames.Count];
            for (int i = 0; i < activeGames.Count; i++)
            {
                _unifiedOrder[i] = activeGames[i].LegacyMode;
            }
        }
        else
        {
            // Fallback default list if registry is empty
            _unifiedOrder = new[]
            {
                TestMode.Reaction,
        ...
        _unifiedRecords = new UnifiedExamSegmentRecord[_unifiedOrder.Length];
    ...
    {
        if (_unifiedOrder == null || _unifiedNextIndex >= _unifiedOrder.Length || _unifiedNextIndex < 0)
        {
            return string.Empty;
        }

        TestMode next = _unifiedOrder[_unifiedNextIndex];
        return GameLocalization.FormatUiString(UiStringKeys.GsiInterstitialBodyFmt, "{0}/{1}, next subject: {2}",
            _unifiedNextIndex + 1, _unifiedOrder.Length, GetUnifiedModeLabel(next));
    }
    ...
    public void CompleteUnifiedExamSegment(UnifiedExamSegmentPayload payload)
    {
        ...
        if (idx < 0 || idx >= _unifiedOrder.Length)
        {
            return;
        }
        ...
        _unifiedNextIndex++;
        if (_unifiedNextIndex < _unifiedOrder.Length)
        {
    ...
    private void FinalizeUnifiedExam()
    {
        ...
        int count = _unifiedOrder != null ? _unifiedOrder.Length : 7;
        for (int i = 0; i < count; i++)
        {
            if (i < _unifiedRecords.Length && _unifiedRecords[i] != null)
            {
    ...
    private string[] CollectUnifiedSegmentSummaryLines()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            lines[i] = (i < _unifiedRecords.Length && _unifiedRecords[i] != null) ? _unifiedRecords[i].SummaryLine : string.Empty;
        }
