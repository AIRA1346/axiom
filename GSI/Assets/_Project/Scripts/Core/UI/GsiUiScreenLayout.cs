...
public static class GsiUiScreenLayout
{
    ...
    public const float HeaderStripHeight = 96f;

    public static readonly RectOffset HeaderStripPadding = new RectOffset(36, 36, 16, 16);

    public const float HeaderRowSpacing = 16f;

    /// <summary>헤더 중앙 화면 제목(Shop / Inventory / 로비 브랜딩 등).</summary>
    public const float ScreenTitleFontSize = 34f;

    /// <summary>화면 제목 자간(크롬을 덜 강조).</summary>
    public const float ScreenTitleCharacterSpacing = 1.2f;

    /// <summary>골드·응시권 한 줄(헤더 내 또는 SubBar).</summary>
    public const float EconomyLineFontSize = 23f;

    public const float SubBarHeight = 54f;

    /// <summary>서브바를 헤더 바로 아래에 붙일 때 Y 앵커 오프셋.</summary>
    public const float SubBarOffsetBelowHeader = -96f;

    public static readonly RectOffset SubBarPadding = new RectOffset(48, 48, 6, 14);
    ...
    public static float ScrollContentTopMargin => HeaderStripHeight + SubBarHeight + 24f; // Clean 24f gap for premium breathing room

    public const float ListVerticalSpacing = 16f; // More airy spacing (16px)

    public static readonly RectOffset ListPadding = new RectOffset(16, 16, 16, 20); // Aligned padding

    /// <summary>상점·인벤 리스트 행 제목 글자 크기.</summary>
    public const float ListRowTitleFontSize = 22f; // Refined slightly

    /// <summary>상점 상품 행(응시권·스킨) 기본 높이.</summary>
    public const float ShopOfferRowPreferredHeight = 94f; // Elegantly taller

    /// <summary>상점 상품 행 내부 여백.</summary>
    public static readonly RectOffset ShopOfferRowPadding = new RectOffset(32, 32, 16, 16);

    /// <summary>인벤토리 스킨 행 기본 높이(상점과 시각적 정렬).</summary>
    public const float InventorySkinRowPreferredHeight = 94f;

    /// <summary>인벤토리 스킨 행 내부 여백.</summary>
    public static readonly RectOffset InventorySkinRowPadding = new RectOffset(32, 32, 16, 16);

    /// <summary>로비 헤더 바 안쪽 <c>HeaderInner</c> 여백(Shop 헤더 HLG 패딩과 시각적으로 맞춤).</summary>
    public static readonly Vector2 LobbyHeaderInnerOffsetMin = new Vector2(36f, 16f);

    public static readonly Vector2 LobbyHeaderInnerOffsetMax = new Vector2(-36f, -16f);
