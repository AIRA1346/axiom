...
public sealed class IntroController : MonoBehaviour
{
    ...
    private void TryCreateRuntimeTagline()
    {
        ...
        tmp.text = GameLocalization.GetUiString(UiStringKeys.IntroTagline,
            "Skill metrics: assessment and records");
        tmp.fontSize = 34f;
        tmp.characterSpacing = 1.15f;
        tmp.alignment = TextAlignmentOptions.Center;
        tmp.color = new Color(GsiUiAppearance.TextSecondary.r, GsiUiAppearance.TextSecondary.g, GsiUiAppearance.TextSecondary.b, 0f);
    ...
    private void ApplyArcaneIntroChrome()
    {
        Canvas canvas = _logoText != null ? _logoText.canvas : (_statusText != null ? _statusText.canvas : null);
        if (canvas != null)
        {
            GsiArcaneUi.EnsureCanvasBackdrop(canvas.transform);
        }

        if (_logoText != null)
        {
            _logoColorOpaque = GsiUiAppearance.TextPrimary;
            _logoText.color = new Color(_logoColorOpaque.r, _logoColorOpaque.g, _logoColorOpaque.b, _logoText.color.a);
            _logoText.characterSpacing = 2.4f;
        }

        if (_statusText != null)
        {
            _defaultStatusColor = GsiUiAppearance.TextSecondary;
            _statusText.color = new Color(_defaultStatusColor.r, _defaultStatusColor.g, _defaultStatusColor.b, _statusText.color.a);
        }
    }
