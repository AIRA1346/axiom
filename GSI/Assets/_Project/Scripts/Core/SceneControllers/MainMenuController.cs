...
public sealed class MainMenuController : MonoBehaviour
{
    ...
    // (Lobby clock fields have been extracted to LobbyClockController)
    ...
    private void EnsureLobbyTopLeftTitleAndClock()
    {
        ...
        GameObject barGo = (barTf == null) ? new GameObject(LobbyTopLeftBarName, typeof(RectTransform)) : barTf.gameObject;
        if (barTf == null) barGo.transform.SetParent(_lobbyRoot, false);

        var clock = barGo.GetComponent<LobbyClockController>() ?? barGo.AddComponent<LobbyClockController>();
        clock.InitializeClockBar();
    }
    ...
    private void ApplyLobbyChrome()
    {
        ...
        UpdateEconomyTexts();
        Transform barTf = _lobbyRoot != null ? _lobbyRoot.Find(LobbyTopLeftBarName) : null;
        if (barTf != null && barTf.TryGetComponent(out LobbyClockController clock))
        {
            clock.RefreshTitleAndTime();
        }
        BringLobbyInteractiveUiInFront();
    }
