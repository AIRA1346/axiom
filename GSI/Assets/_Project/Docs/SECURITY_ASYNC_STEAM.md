...
- `GsiSteamCloudSync`가 **베스트 기록·통합 시험 텍스트/JSON·연습 급수·골드/응시권·볼륨·언어·외관 모드·보유/장착 스킨** 등을 JSON 스냅샷으로 **Steam Remote Storage**에 올리고, 더 최신 스냅샷이 있으면 부팅 시 `PlayerPrefs`에 병합합니다.
...
- [ ] App ID `1628285`의 Steam Cloud 설정에 `gsi_steamcloud_v1.json` 저장이 허용되는지
- [ ] 로컬 QA `steam_appid.txt`는 `1628285`인지
- [ ] 출시 Depot에는 `steam_appid.txt`가 없는지
