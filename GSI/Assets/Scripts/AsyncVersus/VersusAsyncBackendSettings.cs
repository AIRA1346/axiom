...
public static class VersusAsyncBackendSettings
{
    ...
    public static string BaseUrl => PlayerPrefs.GetString(PrefsBaseUrl, string.Empty);

    public static string RelativePath
    {
        get
        {
            string p = PlayerPrefs.GetString(PrefsRelativePath, string.Empty);
            return string.IsNullOrEmpty(p) ? DefaultRelativePath : p;
        }
    }

    public static string ApiKey => PlayerPrefs.GetString(PrefsApiKey, string.Empty);

    public static int TimeoutSeconds => Mathf.Clamp(PlayerPrefs.GetInt(PrefsTimeoutSec, 25), 5, 120);

    /// <summary>true면 <c>Authorization: Bearer …</c>, false면 <c>X-Api-Key</c>.</summary>
    public static bool UseBearerAuth => PlayerPrefs.GetInt(PrefsAuthBearer, 1) != 0;
    ...
    public static string BuildEndpointUrl()
    {
        ...
        return BuildEndpointUrlFromParts(BaseUrl, PlayerPrefs.GetString(PrefsRelativePath, string.Empty));
    }
    ...
    public static void SaveAll(
        ...
        bool useBearerAuth)
    {
        PlayerPrefs.SetString(PrefsBaseUrl, baseUrl?.Trim() ?? string.Empty);
        PlayerPrefs.SetString(PrefsRelativePath, relativePath?.Trim() ?? string.Empty);
        PlayerPrefs.SetString(PrefsApiKey, apiKey ?? string.Empty);
        PlayerPrefs.SetInt(PrefsTimeoutSec, Mathf.Clamp(timeoutSeconds, 5, 120));
        PlayerPrefs.SetInt(PrefsAuthBearer, useBearerAuth ? 1 : 0);
        PlayerPrefs.Save();
    }
    ...
    {
        PlayerPrefs.SetString(PrefsBaseUrl, string.Empty);
        PlayerPrefs.Save();
    }
    ...
    {
        PlayerPrefs.DeleteKey(PrefsBaseUrl);
        PlayerPrefs.DeleteKey(PrefsRelativePath);
        PlayerPrefs.DeleteKey(PrefsApiKey);
        PlayerPrefs.DeleteKey(PrefsTimeoutSec);
        PlayerPrefs.DeleteKey(PrefsAuthBearer);
        PlayerPrefs.Save();
    }
    ...
    {
        PlayerPrefs.SetString(PrefsBaseUrl, baseUrl ?? string.Empty);
        if (relativePathOrNull != null)
        {
            PlayerPrefs.SetString(PrefsRelativePath, relativePathOrNull);
        }
        ...
        {
            PlayerPrefs.SetInt(PrefsTimeoutSec, timeoutSeconds.Value);
        }
