...
public sealed class CodexController : MonoBehaviour
{
    ...
    private List<CodexMetadata> _fullFlatIndex = new List<CodexMetadata>();
    ...
    private void RefreshIndex()
    {
        ...
        if (manager == null)
        {
            _fullFlatIndex.Clear();
            _indexScrollView.SetEntries(new List<CodexMetadata>());
            return;
        }

        manager.BuildIndexIfNeeded();
        IReadOnlyList<CodexMetadata> flat = manager.GetFlatIndexForNavigation();
        _fullFlatIndex.Clear();
        if (flat != null)
        {
            foreach (CodexMetadata e in flat)
            {
                _fullFlatIndex.Add(e);
            }
        }
    ...
    {
        List<CodexMetadata> filtered = FilterByExpanded(_fullFlatIndex);
