...
/// ARCHÉ 전체 빌드: ItemDatabase 로딩 완료 시까지 대기 후 ACCESS GRANTED → 메인 씬 전환.
/// G.S.I 단독(<c>GSI_PRODUCT</c>): 아이템 DB 대기 없이 시험 시설 접속 연출만 합니다.
...
public sealed class IntroController : MonoBehaviour
{
    ...
    private IEnumerator IntroSequence()
    {
        ...
        yield return StartCoroutine(WaitForLocalizationBoot());
...
#if GSI_PRODUCT
        yield return StartCoroutine(TypewriterEffect("ASSESSMENT SYSTEM STANDBY..."));
        yield return new WaitForSecondsRealtime(_phaseDisplayDuration);
#else
        ...
        int itemCount = ItemDatabase.Instance != null ? ItemDatabase.Instance.MetadataEntryCount : 0;
        if (itemCount <= 0)
        {
            itemCount = 100000;
        }
