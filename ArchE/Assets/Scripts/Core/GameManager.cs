...
public sealed class GameManager : MonoBehaviour
{
    ...
    public bool TryStartUnifiedExam(int examGrade1to9)
    {
        ...
        SetGameState(GameState.TestStandby);
        return true;
    }
    ...
    public string GetUnifiedExamInterstitialText()
    {
        ...
        return $"{_unifiedNextIndex + 1}/6 · 다음 과목: {GetUnifiedModeLabel(next)}";
    }
    ...
    private void FinalizeUnifiedExam()
    {
        ...
        string logLine = BuildUnifiedExamLogLine(total, overallPass, minTotal);
    ...
    private string BuildUnifiedExamLogLine(float total, bool overallPass, float minTotal)
    {
        ...
        sb.Append(" · ");
        sb.Append(overallPass ? "최종 합격" : "최종 불합격");
        sb.Append(" · ");
        for (int i = 0; i < 6; i++)
        {
            if (_unifiedRecords[i] != null)
    ...
    /// <summary>G.S.I 씬을 떠날 때(ARCHÉ 복귀 등): 미완료면 세션 폐기, 결과 표시 직후였으면 정리만 합니다.</summary>
