...
public sealed class GSISceneBootstrap : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (GameManager.Instance == null)
        {
            ...
            {
                Debug.LogWarning(
                    "[GSISceneBootstrap] GameManager 오브젝트는 있으나 싱글톤 Instance가 비어 있습니다. 허브(SampleScene)에 DontDestroyOnLoad용 GameManager가 한 개만 있는지 확인하세요.");
            }
            else
            {
                Debug.LogWarning(
                    "[GSISceneBootstrap] GameManager가 없습니다. Intro 또는 SampleScene(허브)을 먼저 거친 뒤 G.S.I로 들어오세요.");
            }
